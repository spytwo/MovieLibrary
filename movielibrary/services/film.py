...
class FilmService:
    ...
    async def get_films_list(self, **filters) -> list[Film]:
        return list(await self.repo.get_multi(**filters))

    async def get_latest_films_for_index(self, limit: int = 5) -> list[Film]:
        return list(await self.repo.get_multi(limit=limit))
    ...
    async def get_paginated_films(
        ...
        films = await self.repo.get_multi(limit=page_size, offset=offset, **filters)
